...
import static com.wdullaer.materialdatetimepicker.time.TimePickerDialog.Version;
...
public class TimePickerFragment extends Fragment implements
    ...
    public static TimePickerFragment newInstance(int hourOfDay, int minute, int second, boolean is24HourMode) {
        TimePickerFragment ret = new TimePickerFragment();
        ret.initialize(hourOfDay, minute, second, is24HourMode);
        return ret;
    }

    public static TimePickerFragment newInstance(int hourOfDay, int minute, boolean is24HourMode) {
        return TimePickerFragment.newInstance(hourOfDay, minute, 0, is24HourMode);
    }

    @SuppressWarnings({"unused", "SameParameterValue"})
    public static TimePickerFragment newInstance(boolean is24HourMode) {
        Calendar now = Calendar.getInstance();
        return TimePickerFragment.newInstance(now.get(Calendar.HOUR_OF_DAY), now.get(Calendar.MINUTE), is24HourMode);
    }

    public void initialize(int hourOfDay, int minute, int second, boolean is24HourMode) {
     ...
     * {@link TimePickerFragment#setSelectableTimes(Timepoint[])}
     ...
     * This is a convenience wrapper around {@link TimePickerFragment#setSelectableTimes(Timepoint[])}
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        int viewRes = R.layout.mdtp_time_picker_fragment;
...
//        else
    ...
    private int deleteLastTypedKey() {
        return mTypedTimes.remove(mTypedTimes.size() - 1);
    }
